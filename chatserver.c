...
int main(int argc, char** argv)
{
	...
	socklen_t addrLen;
	int nClient=0,i=0,port; // i is number of client
	...
	while(1)
	{
		...
		{
			if(i==res) continue;
			sendto(mySock,nameBuff,strlen(nameBuff),0,(struct sockaddr *) &sockets[i],addrLen);
			sendto(mySock,buff,nRecv,0,(struct sockaddr *) &sockets[i],addrLen);
		}
	}
	return 0;
}
...
int checkSockList(struct sockaddr_in *entry,struct sockaddr_in *list,int count)
{
	...
	{
		ptrSockAddr=list +1;
		if(memcmp(ptrSockAddr, entry,sizeof(struct sockaddr_in))==0)
			return i;
	}
	return i;
}
