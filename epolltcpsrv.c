...
int main(int argc,char **argv)
{
	...
	ev.events=EPOLLIN;
	ev.data.fd=listenSd;
	if(epoll_ctl(epfd,EPOLL_CTL_ADD,listenSd,&ev)==-1) errProc("epoll_ctl");
	...
	while(1)
	{
		...
		for(i=0;i<ready;i++)
		{
			if(events[i].data.fd==listenSd)
			{
				...
				if(connectSd==-1)
				{
					fprintf(stderr,"accept errpr");
					continue;
				}
				fprintf(stderr,"a clinet is connected...\n");
			...
			else
			{
				...
				if(readLen==0)
				{
					...
					if(epoll_ctl(epfd,EPOLL_CTL_DEL,readfd,&ev)==-1) errProc("epoll_ctl");
					close(readfd);
					continue;
				}
				rBuff[readLen]='\0';
